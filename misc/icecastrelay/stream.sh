...
ffmpegAuBg(){
bash -c "./ffmpeg_g -f alsa -copytb 1 -correct_ts_overflow 0 -use_wallclock_as_timestamps 1 -i pulse -filter_complex aresample=44100 -acodec pcm_s16le -f wav -ac 2 - 1>/tmp/ffmpegAudio.fifo"  >./test 2>&1  &
#bash -c "./ffmpeg_g -f alsa -copytb 1 -correct_ts_overflow 0 -use_wallclock_as_timestamps 1 -i pulse -filter_complex aresample=44100 -acodec flac -f flac -ac 2 - 1>/tmp/ffmpegAudio.fifo"  >./test 2>&1  &
echo $!
}

ffmpegViBg(){
bash -c "./ffmpeg_g -f x11grab -s "$INRES" -r $FPS -use_wallclock_as_timestamps 1 -i :0.0 -vcodec libx264 -preset ultrafast -qp 0 -f flv - 1>/tmp/ffmpegVideo.fifo"  >./test2 2>&1  &
echo $!
}

ffmpegBg (){
 ./ffmpeg_g -loglevel panic -stats -use_wallclock_as_timestamps 1 -i "/tmp/ffmpegAudio.fifo" -use_wallclock_as_timestamps 1 -i "/tmp/ffmpegVideo.fifo" -f ogg -ac 2 \
...
SOURCE_DUPLEX_ID=$(pactl list short sources | mawk '$2 ~ "duplex_out" {print $1}')
echo "${FFMPEG_ID} ${FFMPEG_STREAM_ID} ${SOURCE_DUPLEX_ID}"
...
read -n 1
kill -s SIGKILL $(pgrep -P $ffmpegAuPID)
kill -s SIGKILL $(pgrep -P $ffmpegViPID)

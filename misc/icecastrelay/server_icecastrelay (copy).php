...
/*
while (!feof($fp)){
$ret .= fread($fp, 10*(1024*1024));
}
*/
while (true){
 $oldvalue=$ret;
 if($oldvalue != $ret .= fread($fp, 10*(1024*1024))){break;}
 if(feof($fp)){die('Connecting to icecast faild');}
}
if (!$ret == "HTTP/1.0 200 OK"){die('Connecting to icecast faild');}
//echo $ret;
...
{
 fread($fp, 1);
 if (feof($fp)){die('icecast dropped connection');}
